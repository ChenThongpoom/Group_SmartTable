...
def condEye(pipeCamRecv,pipeLinRecv3,pipeMainSend4):
    cond = ''
    count = 0
    yVal = pipeCamRecv.recv()  #pipe receive distance from main function to check the condition with the leftEye value
    while True:
        test = pipeLinRecv3.recv()  # pipe receive the leftEye value from the main function
        print(yVal,' is y val')
        print("Test is ", test)
...
def main(pipeDisRecv2,pipeCamSend):
    p3.start()
    p3.join()
    Dis = pipeDisRecv2.recv()
    if type(Dis) == int:
        p1.start()
        p2.start()
        p1.join()
...
#     y = distanceUs1()
    ...
    p2 = Process(target=condEye,args=(pipeEyeRecv,pipeEyeRecv3,pipeEyeSend4))
